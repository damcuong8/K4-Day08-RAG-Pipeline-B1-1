...
LLM_MODEL = "google/gemma-4-31b-it:free"  # model free trên OpenRouter (confirmed available)
...
def generate_with_citation(query: str, top_k: int = TOP_K) -> dict:
    ...
    if not api_key:
        raise ValueError("Missing API key: OPENROUTER_API_KEY or OPENAI_API_KEY is not set.")
    ...
    try:
        ...
        # Fallback khi model trả về rỗng hoặc lỗi API (rate limit, empty output, v.v.)
        print(f"[LLM warning] {llm_err}")
        answer = "Tôi không thể xác minh thông tin này từ nguồn hiện có"
